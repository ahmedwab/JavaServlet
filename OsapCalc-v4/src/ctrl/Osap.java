 ...
 */
@WebServlet({ "/Osap", "/Osap/*","/Osap/Ajax" })
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String graceperiod = "on";
		 ...
		 if(requestedPrincipal!=null) {
			...
			request.setAttribute("principal", requestedPrincipal);
		 }
		 ...
		 if(requestedGrace!=null) {
			 ...
			 session.setAttribute("graceperiod",requestedGrace);
			 
		 }
		 System.out.println(session.getAttribute("graceperiod"));
		 ...
		 Double monthlyPayments = l.computePayment(interest, principal, gracePeriod, grace, graceinterest);
		 ...
		 l=(Loan) request.getAttribute("mLoan");
		  ...
		  if(request.getParameter("calculate")==null || graceinterest<0) {
			 ServletContext context = getServletContext();
			  ...
			  if (request.getParameter("calculate").equals("false") ) {
				  System.out.println("ajax request");
			  resOut.write("<div>Grace Period Interest: $"+df.format(graceinterest)+"</div><br>");
			  resOut.write("Monthly Payment: $"+df.format(monthlyPayments));
			  }
			  else {
